...
function initMap() {
        map = new google.maps.Map(document.getElementById('map-canvas'), {
          center: {lat: 24.3133, lng: 52.6000},
          mapTypeId: google.maps.MapTypeId.HYBRID,
          disableDefaultUI: true,
          tilt:90,
          zoom: 14
...
SC.stream("/tracks/209403849", function(sound){
      birdSound = sound;
});
...
SC.stream("/tracks/209403842", function(sound){
      secondSound = sound;
});
...
socket.on('playSongA', function (data) {
  if()
  secondSound.stop();
  birdSound.play();
})

socket.on('playSongB', function (data) {
